...
./$SCRIPT -y hadrosaur -r tyrannosaur > wrk/1
diff wrk/1 ref/1
if [ $? -ne 0 ]; then
	echo "Failure"
else
...
./$SCRIPT -y stegosaur -r allosaur > wrk/2
diff wrk/2 ref/2
if [ $? -ne 0 ]; then
        echo "Failure"
else
...
./$SCRIPT -y hadrosaur -y ceratop -r tyrannosaur > wrk/3
diff wrk/3 ref/3
if [ $? -ne 0 ]; then
        echo "Failure"
else
...
./$SCRIPT -y hadrosaur -r tyrannosaur -r raptor -y ceratop -r allosaur > wrk/4
diff wrk/4 ref/4
if [ $? -ne 0 ]; then
        echo "Failure"
else
