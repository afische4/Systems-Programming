...
void RxGetDataCallBack(tRx* pRx, uint8 RxData)
{
  ...
  uchar c = RxData;
  pRxTest->adt = &c;
  //  fwrite(&c, 1, 1, pRxTest->adt);
}


int tx(char *msg, char **adt, int msglen){

//adt is the address of a char* right now;
//msg is the message to be encoded
//text to audio
    int adtlen = 50000;
    uint SampleRate = 8000;
    double NoiseLevel = 0.0;
    int res;  
    //instead of file, just get the message, no reading from file at all, instead just pass the message
    *adt = malloc(adtlen * sizeof(char)); //ptr to the space, so where the audio data is written
    //res = testTx(sampleRate, noiseLevel, 
    tTxTest txTest;
    int count =0;
    if(msg == NULL){
        return -1;
    ...
    txTest.msg = msg;
    txTest.CountDown = 0;
    TxInit(&txTest.Tx, SampleRate, &TxGetDataCallBack);
      ...
      do
      {
        ...
        //fprintf(adt,&sample, 1, sizeof(sample), adt);
...
//msg is &nmg which is a new pointer to a char *
//adt is a pointer to a buffer of audio data of length adtlen
//audio to text
    int SampleRate = 8000;
  uint lastState;
  ...
  for (i = 0; i < adtlen; i++)
  {
    int16 sample;
    count++;
    if (*(adt + count*sizeof(sample)) != sizeof(sample))
    {
...
tComplex complexAdd(const tComplex* a, const tComplex* b); //returns addition of struct tcomplex

tComplex complexMul(const tComplex* a, const tComplex* b); //returns mult of struct tcomples

void dft(tComplex out[], const tComplex in[], size_t n, int direction); //no return, but does the audio transform

void TxInit(tTx* pTx,uint SampleRate, uint (*pTxGetDataCallBack)(tTx*, uint8*)); //dontt worry about this

int16 TxGetSample(tTx* pTx); //returns the sample, don't change

void RxInit(tRx* pRx, uint SampleRate, void (*pRxGetDataCallBack)(struct tRx*, uint8)); //dont' worry about this

void RxGetSample(tRx* pRx, int16 Sample); //dont' change

uint TxGetDataCallBack(tTx* pTx, uint8* pTxData); //don't change this

int testTx(uint SampleRate, double NoiseLevel, const char* DataFileName, const char* AudioFileName);//transforms the data reading and writing to files. change this!!!! call in tx

void RxGetDataCallBack(tRx* pRx, uint8 RxData);//don't change

int testRx(uint SampleRate, const char* AudioFileName, const char* DataFileName);//change this to deal with ptrs instead of files!! call in rx
