...
void save_dino(dino *d, char *ofn)
{
    ...
    fwrite(d->name, sizeof(char), (*d).namelen, fp);
    fclose(fp);
}
...
void load_dino(dino *d, char *ifn)
{
    ...
    fread(&(d->namelen), sizeof(int), 1, fp);
    (*d).name = malloc((*d).namelen * sizeof(char)+1);
    (*d).name[(*d).namelen] = '\0';
    fread(d->name, sizeof(char), (*d).namelen, fp);
    fclose(fp);
}
...
int readdinos(char *fn, dino **dinos)
{
    ...
    while(n!=-1)
    {
        ...
        if (numsplits == 27)
        {
            ...
            //(*dinos[i]).name[(*dinos[i]).namelen] = '\0';
            i++;
        }
        memset(buf, 0, sizeof(char) * max + 1);    
