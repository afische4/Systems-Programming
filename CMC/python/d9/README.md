...
On the exam, you wrote a script in bash to calculate the prey-predator ratio among different animals.  You will do the same thing on this assignment!  Except for this daily, you only need to support one predator and one prey animal, like so:
...
Note this is *not* the same test script as the exam.  There are subtle differences in the way awk and Python regex works, so the results may be off by one or two between the bash and Python versions.  Also, again, we're not using the -y or -r flags yet.
