...
cat $PASS | sed -nE "s/^.*([0-9]{$NLEN}).*$/\1/p" | sort | uniq -c | sort -r -h | head -n $MAX

echo ""
echo "top $MAX most-common words of length $WLEN"

cat $PASS | sed -nE "s/^[^a-z]*([a-z]{$WLEN})[^a-z]*$/\1/p" | sort | uniq -c | sort -r -h | head -n $MAX
...
NUMLNL=$(cat $PASS | sed -nE "s/^([a-zA-Z])+[0-9]+[a-zA-Z]+$//p" | wc -l )
echo ""
echo "top $MAX most-common of pattern letters-numbers-letters (out of $NUMLNL total)"

cat $PASS | sed -nE "s/(^[a-zA-Z]+[0-9]+[a-zA-Z]+)/\1/p" | sort | uniq -c | sort -r -h | head -n $MAX

echo ""
echo "number of total (non-unique) passwords that match pattern: my NUM XYZ"

cat $PASS | sed -nE "s/^my[0-9]+[a-zA-Z]{2,}$//p" | sort | uniq -c
...
do
    NUMSEQ=$(cat $PASS | sed -nE "/^.*[0-9]{$i}.*$/p" | wc -l)
    echo "$i $NUMSEQ"
done
