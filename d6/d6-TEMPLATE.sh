...
get_lnglat(){
    ...
    LAT= # use awk to get latitude from zipcodes.dat
    
    # get longitude of city using awk (same as above, just a different column
    LNG= # use awk to get longitude from zipcodes.dat
}
...
set_awkprog(){
    ...
    AWKPROG="NR>1 {print}" # modify this simple program to check for min/max lat/lng
}
