...
get_lnglat(){
    ...
    LAT=$(cat zipcodes.dat | awk -F ';' -v OFS=';' '{if (NR>1 && $1=="'$ZIPCODE'") {print $4; }}')
    # use awk to get latitude from zipcodes.dat
    
    # get longitude of city using awk (same as above, just a different column
    LNG=$(cat zipcodes.dat | awk -F ';' -v OFS=';' '{if (NR>1 && $1=="'$ZIPCODE'") {print $5; }}')   
}
...
set_awkprog(){
    ...
    AWKPROG="NR>1 && \$20>=$MIN_LAT && \$20<=$MAX_LAT && \$19>=$MIN_LNG && \$19<=$MAX_LNG {  print ; }"
    # modify this simple program to check for min/max lat/lng
}
